...
void segv_thunk_handler(int cause, siginfo_t *info, void *uap) {
    ...
    if (info->si_addr >= (void *)_env__table_ &&
        ...
        // Test/debug only (otherwise I/O should be avoided in a signal
        // handlers)
...
#include <stdarg.h>
int _env___printf_(const char *fmt, va_list args) { vprintf(fmt, args); }

void _env__abortStackOverflow_(uint32_t x) {
    FATAL("_env__abortStackOverflow 0x%x\n", x);
}

void _env__nullFunc_X_(uint32_t x) { FATAL("_env__nullFunc_X_ 0x%x\n", x); }
