...
char *value_repr(StackValue *v) {
    ...
    case I32:
        ...
        break;
    case I64:
        snprintf(_value_str, 255, "0x%llx:i64", v->value.uint64);
        break;
    case F32:
...
Block *pop_block(Module *m) {
    ...
    if (t->result_count == 1) {
        if (m->stack[m->sp].value_type != t->results[0]) {
            exception(EX_CALL_SIG_MISMATCH);
            return NULL;
        }
    }
...
void thunk_out(Module *m, uint32_t fidx) {
    ...
    switch (thunk_mask) {
    case 0x800:
        thunk_out_0_0(m, func, type);
        break;
    case 0x8001:
        thunk_out_0_i(m, func, type);
        break;
    case 0x80011:
        thunk_out_0_ii(m, func, type);
        break;
    case 0x8001111:
        thunk_out_0_iiii(m, func, type);
        break;
    case 0x810:
        thunk_out_i_0(m, func, type);
        break;
    case 0x8101:
        thunk_out_i_i(m, func, type);
        break;
    case 0x81011:
        thunk_out_i_ii(m, func, type);
        break;
    case 0x810111:
        thunk_out_i_iii(m, func, type);
        break;
    case 0x8101111:
        thunk_out_i_iiii(m, func, type);
        break;
    case 0x81011111:
        thunk_out_i_iiiii(m, func, type);
        break;
    case 0x80033:
        thunk_out_0_ff(m, func, type);
        break;
    case 0x800333:
        thunk_out_0_fff(m, func, type);
        break;
    case 0x8003333:
        thunk_out_0_ffff(m, func, type);
        break;
    case 0x8303:
        thunk_out_f_f(m, func, type);
        break;
    case 0x800444444:
        thunk_out_0_FFFFFF(m, func, type);
        break;
    case 0x8103:
        thunk_out_i_f(m, func, type);
        break;
    case 0x8404:
        thunk_out_F_F(m, func, type);
        break;
    ...
    while (m->pc < m->byte_count) {
        ...
        case 0x11:                              // call_indirect
            ...
            if (val >= m->table.maximum) {
                exception_arg(EX_UNDEFINED_ELEMENT, val);
                return false;
            }
            ...
            if (fidx < m->import_count) {
                ...
                if (ftype->param_count + func->local_count !=
                    m->sp - m->fp + 1) {
                    exception(EX_INDIRECT_CALL_SIGNATURE_MISMATCH);
                    return false;
                }
        ...
        case 0x40:                       // grow_memory
            ...
            m->memory.bytes =
                arecalloc(m->memory.bytes, prev_pages * pow(2, 16),
                          m->memory.pages * pow(2, 16), sizeof(uint32_t),
                          "Module->memory.bytes");
            continue;
        ...
        case 0x28 ... 0x35:
            ...
            if (!m->options.disable_memory_bounds) {
                if (overflow) {
                    ...
                    exception(EX_OUT_OF_BOUNDS_MEMORY_ACCESS);
                    return false;
                }
            }
        ...
        case 0x6a ... 0x78:
            ...
            // i32.add case 0x6b: o = __builtin_sub_overflow(a, b, &c); break;
            // // i32.sub
            case 0x6a:
                c = a + b;
                break; // i32.add
        ...
        case 0xa8:
            if (isnan(stack[m->sp].value.f32)) {
                exception(EX_INVALID_CONVERSION_TO_INT);
                return false;
        ...
        default:
            exception_arg(EX_UNKNOWN_OPCODE, opcode);
            return false;
        }
    }
...
void run_init_expr(Module *m, uint8_t type, uint32_t *pc) {
    // Run the init_expr
    Block block = {
        .block_type = 0x01, .type = get_block_type(type), .start_addr = *pc};
    m->pc = *pc;
    push_block(m, &block, m->sp);
...
Module *load_module(char *path, Options options) {
    ...
    while (pos < mod_len) {
        ...
        case 2:
            ...
            for (uint32_t gidx = 0; gidx < import_count; gidx++) {
                ...
                DEBUG("  import: %d/%d, external_kind: %d, %s.%s\n", gidx,
                      import_count, external_kind, import_module, import_field);
                ...
                do {
                    ...
                    if (resolvesym(import_module, import_field, &val, &err)) {
                        break;
                    }
                ...
                case 0x03: // Global
                    m->global_count += 1;
                    m->globals = arecalloc(m->globals, m->global_count - 1,
                                           m->global_count, sizeof(StackValue),
                                           "globals");
        ...
        case 3:
            ...
            Block *functions;
            functions =
                acalloc(m->function_count, sizeof(Block), "Block(function)");
            if (m->import_count != 0) {
        ...
        case 9:
            ...
            for (uint32_t c = 0; c < element_count; c++) {
                ...
                if (m->options.mangle_table_index) {
                    // offset is the table address + the index (not sized for
                    // the pointer size) so get the actual (sized) index
                ...
                for (uint32_t n = 0; n < num_elem; n++) {
                    DEBUG("  write table entries %p, offset: 0x%x, n: 0x%x, "
                          "addr: %p\n",
